...
export default function PortalLayout() {
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-50 w-60 bg-[#0f172a] text-slate-200 border-r border-slate-800 transform transition-transform duration-200 ease-in-out
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between h-14 px-4 border-b border-slate-800">
            <div className="flex items-center gap-2.5">
              ...
              <span className="font-bold text-[16px] tracking-tight text-white">Cổng Báo Giá</span>
            </div>
            <button
          ...
          <nav className="flex-1 px-3 py-4 space-y-1">
            {portalNavItems.map((item) => (
              <NavLink
                ...
                onClick={() => setSidebarOpen(false)}
                className={({ isActive }) =>
                  `flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[13px] font-medium transition-all cursor-pointer ${
                    isActive
          ...
          <div className="border-t border-slate-800 p-3 bg-slate-900/50">
            <div className="flex items-center gap-2.5 mb-3 px-1">
              <div className="w-8 h-8 rounded-full bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center shadow-inner">
